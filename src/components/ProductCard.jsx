...
const ProductCard = ({ product }) => {
    ...
    return (
        <Link to={`/product/${product.id}`} className="card product-card flex flex-col h-full bg-slate-900 border border-slate-800 rounded-xl overflow-hidden hover:border-emerald-500/50 transition-all duration-300 group cursor-pointer block">
            <div className="p-6 flex-1 flex flex-col">
                ...
                <div className="space-y-3 mb-6">
                    ...
                    {product.features.length > 3 && (
                        <div className="text-xs text-slate-500 pl-6">
                            + {product.features.length - 3} autres points
                        </div>
                    )}
                </div>
                ...
                <div className="mt-auto flex items-center justify-between pt-4 border-t border-slate-800">
                    ...
                    <span className="btn btn-primary text-sm group-hover:scale-105 transition-transform flex items-center text-white bg-emerald-600 hover:bg-emerald-500 px-4 py-2 rounded-lg font-medium">
